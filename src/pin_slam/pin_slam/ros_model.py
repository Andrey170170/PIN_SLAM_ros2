...
from pin_slam.dataset.slam_dataset import SLAMDataset
...
from pin_slam.utils.loop_detector import NeuralPointMapContextManager, detect_local_loop
from pin_slam.utils.mapper import Mapper
from pin_slam.utils.mesher import Mesher
from pin_slam.utils.pgo import PoseGraphManager
from pin_slam.utils.tools import (
    freeze_decoders,
    get_time,
    load_decoder,
    save_implicit_map,
...
from pin_slam.utils.tracker import Tracker
...
class PINSLAMer(Node):
    def __init__(self):
        ...
        self.declare_parameter("/pin_slam/global_frame_name", "map")
        self.declare_parameter("/pin_slam/body_frame_name", "base_link")
        self.declare_parameter("/pin_slam/sensor_frame_name", "range_sensor")
        self.declare_parameter("/pin_slam/config_path", "/home/andrey/PycharmProjects/PIN_SLAM/config/lidar_slam/run.yaml")
        self.declare_parameter("/pin_slam/point_cloud_topic", "/cepton_pcl2")

        self.global_frame_name = self.get_parameter("/pin_slam/global_frame_name").get_parameter_value().string_value # odom
        self.body_frame_name = self.get_parameter("/pin_slam/body_frame_name").get_parameter_value().string_value
        self.sensor_frame_name = self.get_parameter("/pin_slam/sensor_frame_name").get_parameter_value().string_value # child
        config_path = self.get_parameter("/pin_slam/config_path").get_parameter_value().string_value
        point_cloud_topic = self.get_parameter("/pin_slam/point_cloud_topic").get_parameter_value().string_value
        ...
        # publisher
        queue_size_ = 10
        self.traj_pub = self.create_publisher(nav_msgs.msg.Path, "/pin_slam/pin_path",  qos_profile=queue_size_)
        self.path_msg = nav_msgs.msg.Path()
        self.path_msg.header.frame_id = self.global_frame_name
        self.odom_pub = self.create_publisher(Odometry, "/pin_slam/odometry", qos_profile=queue_size_)
        self.frame_input_pub = self.create_publisher(PointCloud2, "/pin_slam/frame/input", qos_profile=queue_size_)
        self.frame_map_pub = self.create_publisher(PointCloud2, "/pin_slam/frame/mapping", qos_profile=queue_size_)
        self.frame_reg_pub = self.create_publisher(PointCloud2, "/pin_slam/frame/registration", qos_profile=queue_size_)
        self.map_pub = self.create_publisher(PointCloud2, "/pin_slam/map/neural_points", qos_profile=queue_size_)
        self.tf_broadcaster = tf2_ros.TransformBroadcaster(self, qos=queue_size_)

        # self.pose_pub = self.create_publisher("/pin_slam/pose", PoseStamped, qos_profile=100)
        # self.neural_points_pub = self.create_publisher("/pin_slam/neural_points", PointCloud2, qos_profile=10)
        ...
        # ros service
        self.srv_save_results = self.create_service(Empty, '/pin_slam/save_results', self.save_slam_result_service_callback)
        self.srv_save_mesh = self.create_service(Empty, '/pin_slam/save_mesh', self.save_mesh_service_callback)

        # for each frame
        self.frame_sub = self.create_subscription(PointCloud2, point_cloud_topic, self.frame_callback, 10)
        self.logger.info('Waiting for the point cloud rostopic: '+ point_cloud_topic)

    def save_slam_result_service_callback(self, request, response):
        # Do something when the service is called
        self.logger.info("Service called, save results")
        self.save_results(terminate=False)

        return response

    def save_mesh_service_callback(self, request, response):
        self.logger.info("Service called, save mesh")
    ...
    def frame_callback(self, msg):
        ...
        # if the robot stops, also don't process this frame, since there's no new oberservations
    ...
    def check_exit(self):
        ...
        while not self.is_shutdown:
            ...
            if delta_t_s > self.config.timeout_duration_s and self.begin:
                ...
                self.logger.info("Timeout, save results and exit")
                break
            rate.sleep()
        else:
    ...
    def __del__(self):
        ...
        print("[bold green]PIN-SLAM ends[/bold green]","📍" )

def main(args=None):
    rclpy.init(args=args)
    try:
        executor = SingleThreadedExecutor()
        control_node_template = PINSLAMer()
        executor.add_node(control_node_template)
        executor.spin()
